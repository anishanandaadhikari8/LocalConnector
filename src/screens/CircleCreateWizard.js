...
export default function CircleCreateWizard({ navigation }) {
  const [name, setName] = useState('');
  const [type, setType] = useState('APARTMENT');
  const [radius, setRadius] = useState('2');
  const [features, setFeatures] = useState(['TASKBOARD','ANALYTICS']);
  const toggle = (f) => setFeatures((arr)=> arr.includes(f) ? arr.filter(x=>x!==f) : [...arr, f]);
